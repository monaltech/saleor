...
def setup_cash_on_delivery_gateway(settings):
    settings.PLUGINS = ["saleor.payment.gateways.cash_on_delivery.plugin.CashOnDeliveryGatewayPlugin"]
    return settings


def test_authorize_success(payment_cash_on_delivery):
    txn = gateway.authorize(payment=payment_cash_on_delivery, token="Fake")
    assert txn.is_success
    assert txn.kind == TransactionKind.AUTH
    assert txn.payment == payment_cash_on_delivery
    payment_cash_on_delivery.refresh_from_db()
    assert payment_cash_on_delivery.is_active
...
def test_authorize_failed(is_active, charge_status, payment_cash_on_delivery):
    payment = payment_cash_on_delivery
...
def test_authorize_gateway_error(payment_cash_on_delivery, monkeypatch):
    monkeypatch.setattr("saleor.payment.gateways.cash_on_delivery.cash_on_delivery_success", lambda: False)
    with pytest.raises(PaymentError):
        txn = gateway.authorize(payment=payment_cash_on_delivery, token="Fake")
        assert txn.kind == TransactionKind.AUTH
...
def test_void_failed(is_active, charge_status, payment_cash_on_delivery):
    payment = payment_cash_on_delivery
    payment.is_active = is_active
    payment.charge_status = charge_status
    payment.save()
...
def test_capture_failed(
    amount, captured_amount, charge_status, is_active, payment_cash_on_delivery
):
    payment = payment_cash_on_delivery
    payment.is_active = is_active
...
def test_refund_failed(
    initial_captured_amount, refund_amount, initial_charge_status, payment_cash_on_delivery
):
    payment = payment_cash_on_delivery
