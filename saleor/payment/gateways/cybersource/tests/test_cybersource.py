...
def setup_cybersource_gateway(settings):
    settings.PLUGINS = ["saleor.payment.gateways.cybersource.plugin.CyberSourceGatewayPlugin"]
    return settings


def test_authorize_success(payment_cybersource):
    txn = gateway.authorize(payment=payment_cybersource, token="Fake")
    assert txn.is_success
    assert txn.kind == TransactionKind.AUTH
    assert txn.payment == payment_cybersource
    payment_cybersource.refresh_from_db()
    assert payment_cybersource.is_active
...
def test_authorize_failed(is_active, charge_status, payment_cybersource):
    payment = payment_cybersource
...
def test_authorize_gateway_error(payment_cybersource, monkeypatch):
    monkeypatch.setattr("saleor.payment.gateways.cybersource.cybersource_success", lambda: False)
    with pytest.raises(PaymentError):
        txn = gateway.authorize(payment=payment_cybersource, token="Fake")
        assert txn.kind == TransactionKind.AUTH
...
def test_void_failed(is_active, charge_status, payment_cybersource):
    payment = payment_cybersource
    payment.is_active = is_active
    payment.charge_status = charge_status
    payment.save()
...
def test_capture_failed(
    amount, captured_amount, charge_status, is_active, payment_cybersource
):
    payment = payment_cybersource
    payment.is_active = is_active
...
def test_refund_failed(
    initial_captured_amount, refund_amount, initial_charge_status, payment_cybersource
):
    payment = payment_cybersource
